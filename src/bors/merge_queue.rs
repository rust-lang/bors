        ...
        match auto_build.status {
            ...
            BuildStatus::Success => {
                    ...
                    .await
                {
                    ...
                    let (comment, mark_build_as_failed) = match &error {
                        ...
                        _ => {
                            (
                                auto_build_push_failed_comment("unknown error"),
                                // Likely a transient error, do not mark the build as failed so
                    ...
                    ctx.db
                        .set_pr_status(&pr.repository, pr_num, PullRequestStatus::Merged)
                        .await?;
                }
...
mod tests {
    ...
    use crate::{
        ...
        github::CommitSha,
        tests::{
            BorsBuilder, BorsTester, BranchPushError, Comment, GitHubState, WorkflowEvent,
            default_repo_name,
        },
    };
        ...
        run_merge_queue_test(pool, async |tester: &mut BorsTester| {
            ...
            tester
                .modify_repo(&default_repo_name(), |repo| {
                    repo.push_error = Some(BranchPushError::Conflict);
                })
                .await;
            tester.process_merge_queue().await;
            insta::assert_snapshot!(
                tester.get_comment(()).await?,
                @":eyes: Tests passed, but fast-forwarding failed: this PR has conflicts with the `main` branch"
            );
            Ok(())
        })
        .await;
    ...
    async fn auto_build_push_validation_failed_comment(pool: sqlx::PgPool) {
        run_merge_queue_test(pool, async |tester: &mut BorsTester| {
            ...
            tester
                .modify_repo(&default_repo_name(), |repo| {
                    repo.push_error = Some(BranchPushError::ValidationFailed);
                })
                .await;
            tester.process_merge_queue().await;
            insta::assert_snapshot!(
                tester.get_comment(()).await?,
                @":eyes: Tests passed, but fast-forwarding failed: this PR is behind the `main` branch"
            );
            Ok(())
        })
        .await;
    ...
    #[sqlx::test]
    async fn auto_build_push_failed_unexpected_error_comment(pool: sqlx::PgPool) {
        run_merge_queue_test(pool, async |_: &mut BorsTester| {
    ...
    #[sqlx::test]
    async fn auto_build_push_conflict_in_db(pool: sqlx::PgPool) {
        run_merge_queue_test(pool, async |tester: &mut BorsTester| {
            start_auto_build(tester).await?;
            tester
                .workflow_full_success(tester.auto_branch().await)
    ...
    #[sqlx::test]
    async fn auto_build_push_validation_failed_in_db(pool: sqlx::PgPool) {
        run_merge_queue_test(pool, async |tester: &mut BorsTester| {
            start_auto_build(tester).await?;
            tester
                .workflow_full_success(tester.auto_branch().await)
    ...
    #[sqlx::test]
    async fn auto_build_unexpected_push_error_remains_enqueued(pool: sqlx::PgPool) {
        run_merge_queue_test(pool, async |tester: &mut BorsTester| {
            start_auto_build(tester).await?;
            tester
                .workflow_full_success(tester.auto_branch().await)
            ...
            tester.expect_comments((), 1).await;
            tester
                .modify_repo(&default_repo_name(), |repo| repo.push_error = None)
                .await;
            tester.process_merge_queue().await;
            tester.expect_comments((), 1).await;
            Ok(())

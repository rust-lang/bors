...
use super::operations::{
    PullRequestStats, approve_pull_request, create_build, create_pull_request, create_workflow,
    delegate_pull_request, find_build, find_pr_by_build, get_nonclosed_pull_requests,
    get_nonclosed_pull_requests_by_base_branch, get_pr_statistics,
...
impl PgDbClient {
    ...
    pub async fn get_pr_statistics(
        &self,
        repo: &GithubRepoName,
    ) -> anyhow::Result<PullRequestStats> {
        get_pr_statistics(&self.pool, repo).await
    }
}

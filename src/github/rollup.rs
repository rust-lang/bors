    ...
    match find_pending_auto_build(&db, &base_repo, &base_branch).await {
        Ok(Some((pending_auto_build, pending_auto_pr_number))) => {
                ...
                .await?
            {
                write!(
                    body,
                    "\nThis rollup conflicts with pending auto build #{pending_auto_pr_number} and may need to be recreated if it succeeds.\n",
                )?;
            }
        }
        Ok(None) => {}
...
pub mod tests {
    ...
    async fn rollup_pending_auto_build_conflict(pool: sqlx::PgPool) {
        ...
        This rollup conflicts with pending auto build #1 and may need to be recreated if it succeeds.
